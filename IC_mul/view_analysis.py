...
def generate_figures(objs: dict, out_dir: str):
    ...
    # Build each figure independently so that a degenerate dataset (e.g. a
    # constant prediction that yields an empty IC series) does not abort the
    # whole run. Failed figures are reported but skipped.
    builders = [
        ("model_perf", lambda: analysis_model.model_performance_graph(pred_label, show_notebook=False)),
        ("report", lambda: analysis_position.report_graph(report_normal_df, show_notebook=False)),
        ("risk", lambda: analysis_position.risk_analysis_graph(analysis_df, report_normal_df, show_notebook=False)),
        ("score_ic", lambda: analysis_position.score_ic_graph(pred_label, show_notebook=False)),
    ]
    for prefix, builder in builders:
        try:
            _save(builder(), prefix)
        except Exception as exc:  # noqa: BLE001 - report and continue
            print(f"  [warn] skipped figure '{prefix}': {type(exc).__name__}: {exc}")
            print(f"          (likely the prediction signal is degenerate / constant -> IC undefined)")
...
def main():
    ...
    parser.add_argument("--recorder-id", default="cbb2e26b187e4642b849a926b3d4886c", help="mlflow recorder id.")
