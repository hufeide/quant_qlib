...
def _raw_label_series(args, pred_index: pd.MultiIndex) -> pd.Series:
    """用 label_expr 从 qlib 取「原始前向收益」（未截面去均值），对齐到 pred 的
    (datetime, instrument) 索引，供净值/收益类图（fig2/fig6）使用。

    训练标签 NFWD1 在中性化 pkl 里已被截面去均值（每天截面均值恒为 0），直接拿它
    当收益会让 Long-Avg 恒为 1、分组净值都贴着 1；净值计算必须用价格还原的 label_expr
    （Ref($close,-1)/$close-1）。训练侧仍用 NFWD1，不受影响。
    """
    ts, te = [s.strip() for s in args.test.split(",")]
    ret = D.features(D.instruments(MARKET), [LABEL_EXPR],
                     start_time=ts, end_time=te, freq="day")
    ret.columns = ["ret"]
    ret = _as_datetime_instrument(ret)
    return ret["ret"].reindex(pred_index)
...
def save_result_figures(recorder, report: pd.DataFrame, importance: pd.Series,
    ...
    # 收益口径必须用「原始前向收益」label_expr（未截面去均值）：NFWD1 已去均值，直接当
    # 收益会让各组合净值都贴着 1、第 5 组几乎看不出来；用 label_expr 还原真实收益。
    try:
        pred = recorder.load_object("pred.pkl")
        if pred is None:
            raise ValueError("pred.pkl 缺失")
        raw_ret = _raw_label_series(args, pred.index)
        score = pred.iloc[:, 0]
        df = pd.concat([score.rename("score"), raw_ret.rename("ret")], axis=1).dropna()
...
def parse_args():
    ...
    p.add_argument("--n-days", type=int, default=50,
    ...
    p.add_argument("--tcn-backend", choices=["qlib", "dynamic"], default="dynamic",
    ...
    # ---- 动态窗口(dynamic)路径：检查点与权重加载 ----
    p.add_argument("--save-every", type=int, default=5,
                   help="动态窗口(dynamic)路径下，每隔多少个 epoch 保存一次模型权重检查点"
                        "（含优化器状态，用于继续训练）。0 表示不按周期保存（仅保留最终最佳权重）。")
    p.add_argument("--weights-dir", default=None,
                   help="动态窗口(dynamic)路径下检查点保存目录；默认为 "
                        "results/tcn_weights/<experiment>。")
    p.add_argument("--resume-from", default=None,
                   help="动态窗口(dynamic)路径下从指定 .pt 检查点继续训练"
                        "（恢复模型权重 + 优化器状态 + 早停历史，从下一 epoch 接着跑）。")
    p.add_argument("--load-weights", default=None,
                   help="动态窗口(dynamic)路径下跳过训练、直接加载指定 .pt 权重对测试段做推理"
                        "（仍需构建特征以标准化与切片，但不再更新参数）。")
...
def main():
    ...
    if args.load_weights and not dynamic:
        log.warning("--load-weights 仅对 --tcn-backend dynamic 生效，当前后端为 qlib，已忽略该选项。")
    if args.weights_dir is None:
        args.weights_dir = os.path.join(HERE, "results", "tcn_weights", args.experiment)
    if dynamic:
    ...
    with R.start(experiment_name=args.experiment, uri=args.mlflow_uri):
        R.log_params(
            **{
                ...
                "tcn.save_every": args.save_every,
                "tcn.weights_dir": args.weights_dir,
                "tcn.resume_from": args.resume_from,
                "tcn.load_weights": args.load_weights,
            }
        )
        ...
        if dynamic:
            ...
            from tcn_dynamic import _segment_windows, predict_from_weights
            if args.load_weights:
                # 跳过训练，直接加载权重对测试段推理
                log.info(f"load-weights(dynamic)：跳过训练，从 {args.load_weights} 加载权重对测试段推理")
                pred_df, label_df, dyn_model = predict_from_weights(
                    feat_raw, label_raw, n_feat, args, segments, args.load_weights)
                model = dyn_model
