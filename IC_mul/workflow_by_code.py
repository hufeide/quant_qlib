...
三、回测与记账
  TopkDropoutStrategy + SimulatorExecutor，交易成本同样是双边 0.002；
...
# 回测配置
# ---------------------------------------------------------------------------
def build_port_analysis_config(model, dataset, topk=TOPK, n_drop=N_DROP,
                               cost_rate=COST_RATE, bench=BENCHMARK,
                               start_time=SEGMENTS["test"][0], end_time=SEGMENTS["test"][1]):
    """TopkDropoutStrategy + 与训练一致的交易成本（双边 cost_rate）。"""
    return {
        "executor": {
...
def main():
    ...
    port_analysis_config = build_port_analysis_config(
        model, dataset, topk=args.topk, n_drop=args.n_drop, cost_rate=args.cost,
        start_time=segments["test"][0], end_time=segments["test"][1],
    )
